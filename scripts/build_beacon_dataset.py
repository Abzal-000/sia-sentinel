...
Живой клиент ретраит сбои средствами SDK (8 повторов с уважением
Retry-After), но таймауты на общем бесплатном пуле штатны, поэтому
длинный прогон обязан идти ПОД ЖУРНАЛОМ ВОЗОБНОВЛЕНИЯ:
``_CheckpointJournal`` в sia/llm_flow.py (протокол llm-flow-checkpoint/3,
JSONL с fsync на каждый завершённый вызов; CLI-флаг --checkpoint-dir).
Падение процесса теряет максимум текущий вызов; возобновление отказывается
смешивать разные датасеты/конфигурации/чекеры. Без чекпойнта неретраенный
сбой на любом вызове действительно теряет весь прогон — а уменьшить
dataset_size после падения нельзя в любом случае: обязательство фиксирует
размер, и расхождение с ним делает запись недействительной. Поэтому:
сначала проба (она же измеряет реальную задержку на вызов), и только потом
предрегистрация полного датасета.
"""
