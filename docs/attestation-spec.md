...
| `anchor_reference` | string \| null | The verifiable identifier behind an `external-anchor` claim: `rekor:<entry-uuid>:<log-index>` or the 64-hex digest of an RFC 3161 timestamp token. Preregistration **refuses** `external-anchor` without one — a bare anchoring claim is trust-shaped, not proof; a third party can resolve the reference against the external log. `unanchored` needs no reference (it is a confession, not a claim). |
