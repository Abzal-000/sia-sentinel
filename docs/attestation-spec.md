...
| `anchor_reference` | string \| null | The identifier behind an `external-anchor` claim; preregistration **refuses** the claim without it, and bare undigested values are refused too — the form must be self-describing. The two accepted forms carry **different verifiability**, stated as they are: `rekor:<entry-id>:<log-index>` points into a **public append-only log**, so a third party resolves it against Rekor directly (entry id is 64 or 80 hex — the sharded public instance returns tree+leaf concatenated). `rfc3161:<token-digest>` cannot be resolved by anyone by itself: the digest binds the CMS timestamp token that the operator holds and **must publish alongside the record**; verification means hashing that published token and comparing digests — until publication there is nothing to check. `unanchored` needs no reference (it is a confession, not a claim). |
